...
`openai-completions` treats `baseURL` as a prefix and the OpenAI SDK sends requests to `<baseURL>/chat/completions`. `openai-completions-full-url` uses the same Chat Completions request body, streaming parser, tool calls, and replay behavior, but sends the configured `baseURL` as the complete POST URL without a path suffix. The full-URL protocol requires an explicit `baseURL`, rejects URL fragments, and does not support model-list interrogation because no `/models` address can be derived from a complete request endpoint; enter its models by hand.
