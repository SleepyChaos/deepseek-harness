...
> **B/C/D 已完成（§16）**：真实边界集成测试（真实 AgentRegistry + 真实 session-query）、
> 队列收敛冒烟（8 任务 × 5 槽）、live lastActivity（session/event 订阅）；D2 客户端面板
> 因 preset 行不进浏览器 boot 图而范围外（架构结论，留 M2）。
> 实现进度见 §12。
...
- 挂载真实 `SessionStore` + `AgentRegistry` + `AgentDefaultModelConfig` + sqlite
  session-query 引擎 + 脚本化 agent 工厂（真实 session + 真实 Inbox + `setup` 真实调用）；
  仅 `llm`/`agentPresets` 保留 mock（模型路由与 roster 属另一边界）。
- 验证结果：`window_create` 发布真实 agent（`ctx.agents.get` 命中）、任务卡进入真实
  session、`window_read` 折叠**真实 surface 事件**（user/assistant message 来自真实
  engine）、send/close 走真实 get/dispose、`agentPreset` 写入真实 session header。
- 关键签名核对（源码实读，非猜测）：真实 `AgentRegistry.create`（`AgentHandle{agent,
  dispose}`、`setup` 时序、meta 展平到 header）与 `SessionQueryService.readSurface`
  （`{session, capturedThroughSeq, events}`）均与插件结构调用完全吻合。

### 16.2 C — 队列收敛冒烟 ✅（`3e63c2bef3`）

8 任务 × 5 槽位预算的批次生命周期：填满→收割（close）→再派发。断言每步
`active ≤ 5`、峰值恰好触达预算、8 个窗口全部 dispose、最终 `status.active = 0`
（槽位回收 + 预算闸在真实建窗路径上的回归）。

### 16.3 D1 — live lastActivity（session/event 订阅）✅（`26bb692300`）

- 插件订阅 post-commit `session/event` feed（全局监听、fiber 所有），对已跟踪窗口的
  子会话事件 `touchActivity` —— 子 Agent 自主进展即刷新活动时间，供调度主 Agent
  假死判定（§14.6），无需任何 orchestrator 工具调用。
- 语义修正：`window_status` 改为纯观察（不再 bump lastActivity）；活动来源 =
  `window_read`/`window_send`/session/event；`window_status` 输出 `lastActivity`。
- 测试：真实边界下，子会话自主追加 surface 事件后 `lastActivity` 推进。

### 16.4 D2 — Client 监控面板：**范围外（架构结论，留 M2）**

侦察结论：Web 端 client 插件图（`ClientModuleRegistry` / `window.__DSH_BOOT__`）
只枚举**宿主 loader 条目**（profile bundles）且要求包声明 `dsh.client`；**preset 行
的 client 半区不会进入浏览器 boot 图**。因此「窗口监控面板」若挂在并发 preset 上
无法加载；正确归宿是宿主平面行（M2，需 Typert RPC + host 组合行），本阶段不实现，
避免为不可加载功能引入 Typert 代码生成与 client bundle。

### 16.5 能力梯级 config 化 + `{{model}}` 修复（合并分支后）

**真机反馈**：主会话 `window_create` 出的子 Agent 本轮失败
`prompt variable "{{model}}" has no value for this assembly (section "deployment:persona")`。

**根因（源码实读）**：`{{model}}`/`{{provider}}`/`{{cwd}}` 装配变量由 agent-loop 注册，
读取 `agent.options.provider/model` 与 `agent.session.header.cwd`；旧 `window_create`
未传 provider/model 与 cwd 时两者皆缺 → 子 Agent 装配失败。

**修复 + 需求落地**（`level` config 化）：

1. **能力梯级 config 化**（本次需求）：`Config.levels` 三级表
   （`l1/l2/l3 × {preset, provider, model, reasoningEffort}`，默认值按 §7.1，待按环境校准）
   + `defaultLevel`；`window_create(level?, preset?, provider?, model?, reasoningEffort?, …)`。
   解析顺序：**显式 provider/model 对 > level 展开 > `agentDefaultModel` 兜底**；
   preset 同理由 level 展开、缺省 `minimal`。主 Agent 只传 `level: l1|l2|l3` 标记即可，
   §15.4/§15.5 升级流程随之简化为「升到 level: l2/l3」。
2. **cwd 继承**（合并代码已含）：未传 cwd 时继承调用者 `session.header.cwd`（修复 `{{cwd}}`）。
3. 并发 preset（用户根 + 示例）配置 `levels` 表 + `defaultLevel: l1`；编排提示词升级段改传 level。
