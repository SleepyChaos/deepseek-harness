...
| `window_read` | Fetch the child's current model surface as compact text | Reads via `sessionQuery.readSurface()`; folds `user/message`, `assistant/message`, `tool/result` events; falls back to status-only when unreadable |
...
| `window_close` | Stop the child, release the budget slot | Calls `cancel('closed-by-tool')` + `handle.dispose()`; removes entry from registry |
